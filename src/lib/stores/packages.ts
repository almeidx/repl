...
export function installPackage(name: string, version: string = "latest"): Promise<void> {
	...
	if (existing) {
		packages.update((p) => p.map((pkg) => (pkg.name === name ? { ...pkg, version, status: "installing" } : pkg)));
	} else {
	...
	return enqueuePackageOperation(async () => {
		try {
			...
			packages.update((p) =>
				p.map((pkg) => (pkg.name === name ? { ...pkg, version: installedVersion, status: "installed" } : pkg)),
			);
		} catch {
			packages.update((p) => p.map((pkg) => (pkg.name === name ? { ...pkg, status: "error" } : pkg)));
		}
	});
}
...
export function removePackage(name: string): Promise<void> {
	...
	packages.update((p) => p.map((pkg) => (pkg.name === name ? { ...pkg, status: "installing" } : pkg)));
